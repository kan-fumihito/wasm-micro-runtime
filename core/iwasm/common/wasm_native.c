...
quick_sort_symbols(NativeSymbol *native_symbols, int left, int right)
{
    ...
    while (left < right) {
        ...
        while (left < right
               && strcmp(native_symbols[left].symbol, base_symbol.symbol)
                    < 0) {
            left++;
        }
...
void *
wasm_native_resolve_symbol(const char *module_name,
...
#if WASM_ENABLE_WAMR_COMPILER                                                 \
  == 0 /* Output warning except running aot compiler */
                ...
                /* Save signature for runtime to do pointer check and
                   address conversion */
                buffer = wasm_runtime_malloc(strlen(signature));
