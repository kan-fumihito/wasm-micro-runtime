...
void
wasm_runtime_restore_init(RuntimeInitArgs *init_args,
                          size_t malloc_size,
                          size_t free_size)
{
    wasm_memory_set_mode(MEMORY_MODE_POOL);
    wasm_memory_set_heap_addr(init_args->mem_alloc_option.pool.heap_buf);
    wasm_memory_set_heap_size(init_args->mem_alloc_option.pool.heap_size);
    
    set_malloc(malloc_size);
    set_free(free_size);
    if (init_args->n_native_symbols > 0
        && !wasm_runtime_register_natives(init_args->native_module_name,
                                          init_args->native_symbols,
...
bool
wasm_runtime_restore(uint32 **p_argv)
{
    return wasm_restore(p_argv);
}
                              ...
                              char *argv[])
{
    ...
    if (wasm_runtime_is_wasi_mode(module_inst)) {
        ...
        if ((func = wasm_runtime_lookup_wasi_start_function(module_inst)))
            return wasm_runtime_create_exec_env_and_call_wasm(module_inst,
                                                              func, 0, NULL);
...
static bool
parse_function_name(char *orig_function_name,
                    char **p_module_name,
                              ...
                              char *argv[])
{
    ...
    total_size = sizeof(uint32) * (uint64)(cell_num > 2 ? cell_num : 2);
    if ((!(argv1 =
             runtime_malloc((uint32)total_size, module_inst, NULL, 0)))) {
        goto fail;
    }
