...
#define FWRITE_PTR(v, fp)                                                     \
    do {                                                                      \
        unsigned long start_addr = (unsigned long)v;                          \
        fwrite(&start_addr, sizeof(unsigned long), 1, fp);                    \
    } while (0)
#define FREAD_PTR(v, fp) FREAD(v, fp)

#define FREAD_REG(v, fp)                                                      \
    do {                                                                      \
        void *reg;                                                            \
        fread(&reg, sizeof(v), 1, fp);                                        \
        v = reg;                                                              \
    } while (0)
...
_get_time(char *buf)
{
    ...
    local->tm_year += 1900;
    local->tm_mon += 1;
    sprintf(buf, "%d-%d-%d-%d-%d-%d.img", local->tm_year, local->tm_mon,
            local->tm_mday, local->tm_hour, local->tm_min, local->tm_sec);
}

static WASMModuleInstance *restore_module_inst;
static WASMExecEnv *restore_exec_env;
static WASMFunctionInstance *restore_function;
static WASMInterpFrame *restore_frame;
...
#undef HANDLE_OPCODE
    ...
    if (migr_flag) {
        ...
        FILE *fp3, *fp4;
        char buf3[32] = "interp.img";
        char buf4[32] = "args.img";

        if ((fp3 = fopen(buf3, "rb")) == NULL) {
        ...
        /*
            ...
            uint8 *frame_ip_end;
            uint8 opcode;
            uint32 i, depth, cond, count, fidx, tidx, lidx, frame_size;
            uint64 all_cell_num;
            int32 val;
            ...
            uint8 value_type;
          */
        FREAD_PTR(memory, fp3);
        fread(&num_bytes_per_page, sizeof(uint32), 1, fp3);
        FREAD_PTR(global_data, fp3);
        fread(&linear_mem_size, sizeof(uint32), 1, fp3);
        FREAD_PTR(wasm_types, fp3);
        FREAD_PTR(globals, fp3);
        FREAD_PTR(global, fp3);
        fread(&opcode_IMPDEP, sizeof(uint8), 1, fp3);
        FREAD_PTR(frame, fp3);

        FREAD_REG(frame_ip, fp3);
        FREAD_REG(frame_lp, fp3);
        FREAD_REG(frame_sp, fp3);

        FREAD_PTR(frame_csp, fp3);
        FREAD_PTR(cache_items, fp3);
        FREAD_PTR(frame_ip_end, fp3);
        fread(&opcode, sizeof(uint8), 1, fp3);
        fread(&i, sizeof(uint32), 1, fp3);
        fread(&depth, sizeof(uint32), 1, fp3);
        fread(&cond, sizeof(uint32), 1, fp3);
        fread(&count, sizeof(uint32), 1, fp3);
        fread(&fidx, sizeof(uint32), 1, fp3);
        fread(&tidx, sizeof(uint32), 1, fp3);
        fread(&lidx, sizeof(uint32), 1, fp3);
        fread(&frame_size, sizeof(uint32), 1, fp3);
        fread(&all_cell_num, sizeof(uint64), 1, fp3);
        fread(&val, sizeof(int32), 1, fp3);
        FREAD_PTR(else_addr, fp3);
        FREAD_PTR(end_addr, fp3);
        FREAD_PTR(maddr, fp3);
        fread(&local_idx, sizeof(uint32), 1, fp3);
        fread(&local_offset, sizeof(uint32), 1, fp3);
        fread(&global_idx, sizeof(uint32), 1, fp3);
        fread(&local_type, sizeof(uint8), 1, fp3);
        FREAD_PTR(global_addr, fp3);
        fread(&cache_index, sizeof(uint32), 1, fp3);
        fread(&type_index, sizeof(uint32), 1, fp3);
        fread(&cell_num, sizeof(uint32), 1, fp3);
        fread(&value_type, sizeof(uint8), 1, fp3);
        fclose(fp3);

        if ((fp4 = fopen(buf4, "rb")) == NULL) {
            printf("file open error\n");
            return;
        }
        /*
            ...
            WASMInterpFrame *prev_frame;
          */
        FREAD_PTR(module, fp4);
        FREAD_PTR(exec_env, fp4);
        FREAD_PTR(cur_func, fp4);
        FREAD_PTR(prev_frame, fp4);
        fclose(fp4);
        restore_module_inst = module;
        restore_exec_env = exec_env;
    ...
    while (frame_ip < frame_ip_end) {
        ...
        if (migr_count == 500) {
            ...
            char buf1[32] = "static.img";
            char buf2[32] = "mem.img";
            char buf3[32] = "interp.img";
            char buf4[32] = "args.img";
            //_get_time(buf);
            ...
            // static buf
            fwrite(heap_addr, sizeof(char), heap_size, fp1);
            fclose(fp1);
            ...
            static unsigned long g_total_malloc;
            static unsigned long g_total_free;
          */

            FWRITE_PTR(heap_addr, fp2);
            fwrite(&heap_size, sizeof(size_t), 1, fp2);

            size_t malloc_size = get_malloc();
            size_t free_size = get_free();
            fwrite(&malloc_size, sizeof(size_t), 1, fp2);
            fwrite(&free_size, sizeof(size_t), 1, fp2);
            fclose(fp2);

            if ((fp3 = fopen(buf3, "wb")) == NULL) {
                printf("file open error\n");
                return;
            }
            /*
            ...
            uint8 value_type;
          */
            FWRITE_PTR(memory, fp3);
            fwrite(&num_bytes_per_page, sizeof(uint32), 1, fp3);
            FWRITE_PTR(global_data, fp3);
            fwrite(&linear_mem_size, sizeof(uint32), 1, fp3);
            FWRITE_PTR(wasm_types, fp3);
            FWRITE_PTR(globals, fp3);
            FWRITE_PTR(global, fp3);
            fwrite(&opcode_IMPDEP, sizeof(uint8), 1, fp3);
            FWRITE_PTR(frame, fp3);
            FWRITE_PTR(frame_ip, fp3);
            FWRITE_PTR(frame_lp, fp3);
            FWRITE_PTR(frame_sp, fp3);
            FWRITE_PTR(frame_csp, fp3);
            FWRITE_PTR(cache_items, fp3);
            FWRITE_PTR(frame_ip_end, fp3);
            fwrite(&opcode, sizeof(uint8), 1, fp3);
            fwrite(&i, sizeof(uint32), 1, fp3);
            fwrite(&depth, sizeof(uint32), 1, fp3);
            fwrite(&cond, sizeof(uint32), 1, fp3);
            fwrite(&count, sizeof(uint32), 1, fp3);
            fwrite(&fidx, sizeof(uint32), 1, fp3);
            fwrite(&tidx, sizeof(uint32), 1, fp3);
            fwrite(&lidx, sizeof(uint32), 1, fp3);
            fwrite(&frame_size, sizeof(uint32), 1, fp3);
            fwrite(&all_cell_num, sizeof(uint64), 1, fp3);
            fwrite(&val, sizeof(int32), 1, fp3);
            FWRITE_PTR(else_addr, fp3);
            FWRITE_PTR(end_addr, fp3);
            FWRITE_PTR(maddr, fp3);
            fwrite(&local_idx, sizeof(uint32), 1, fp3);
            fwrite(&local_offset, sizeof(uint32), 1, fp3);
            fwrite(&global_idx, sizeof(uint32), 1, fp3);
            fwrite(&local_type, sizeof(uint8), 1, fp3);
            FWRITE_PTR(global_addr, fp3);
            fwrite(&cache_index, sizeof(uint32), 1, fp3);
            fwrite(&type_index, sizeof(uint32), 1, fp3);
            fwrite(&cell_num, sizeof(uint32), 1, fp3);
            fwrite(&value_type, sizeof(uint8), 1, fp3);
            fclose(fp3);

            if ((fp4 = fopen(buf4, "wb")) == NULL) {
                printf("file open error\n");
                return;
            }
            /*
            ...
            WASMInterpFrame *prev_frame;
          */
            FWRITE_PTR(module, fp4);
            FWRITE_PTR(exec_env, fp4);
            FWRITE_PTR(cur_func, fp4);
            FWRITE_PTR(prev_frame, fp4);
            fclose(fp4);
        }
    MIGR_POINT:
        opcode = *frame_ip++;
    ...
    FETCH_OPCODE_AND_DISPATCH();
        ...
        handle_op_block:
            cache_index =
              ((uintptr_t)frame_ip) & (uintptr_t)(BLOCK_ADDR_CACHE_SIZE - 1);
        ...
        handle_op_if:
            ...
            if (cache_items[0].start_addr == frame_ip) {
                ...
                end_addr = cache_items[1].end_addr;
                       ...
                       (BlockAddr *)exec_env->block_addr_cache, frame_ip,
                       (uint8 *)-1, LABEL_TYPE_IF, &else_addr, &end_addr)) {
        ...
        label_pop_csp_n:
           ...
           */
              ...
              :
            {
                ...
                if (local_offset & 0x80)
                    PUT_I64_TO_ADDR(
                      (uint32 *)(frame_lp + (local_offset & 0x7F)), POP_I64());
                else
                    *(int32 *)(frame_lp + local_offset) = POP_I32();
                HANDLE_OP_END();
            }
              ...
              :
            {
                ...
                switch (opcode) {
                    ...
                    case WASM_OP_I32_TRUNC_SAT_U_F32:
                        DEF_OP_TRUNC_SAT_F32(-1.0f, 4294967296.0f, true,
                                             false);
                        break;
                    ...
                    case WASM_OP_DATA_DROP:
                    {
                        ...
                        read_leb_uint32(frame_ip, frame_ip_end, segment);
                        module->module->data_segments[segment]->data_length =
                          0;
                    ...
                    case WASM_OP_TABLE_INIT:
                    {
                        ...
                        bh_memcpy_s(
                          ...
                          module->module->table_segments[elem_idx].func_indexes
                            + s,
                          n * sizeof(uint32));
                    ...
                    case WASM_OP_TABLE_COPY:
                    {
                        ...
                        /* merge all together */
                        bh_memcpy_s((uint8 *)(dst_tbl_inst)
                                      + offsetof(WASMTableInstance, base_addr)
                                      + d * sizeof(uint32),
                                    (dst_tbl_inst->cur_size - d)
                                      * sizeof(uint32),
                                    (uint8 *)(src_tbl_inst)
                                      + offsetof(WASMTableInstance, base_addr)
                                      + s * sizeof(uint32),
                    ...
                    case WASM_OP_TABLE_GROW:
                    {
                        ...
                        if (!wasm_enlarge_table(module, tbl_idx, n,
                                                init_val)) {
                            PUSH_I32(-1);
                        }
                        else {
                ...
                switch (opcode) {
                    ...
                    case WASM_OP_ATOMIC_I32_LOAD16_U:
                    {
                        ...
                        if (opcode == WASM_OP_ATOMIC_I32_LOAD8_U) {
                            CHECK_BULK_MEMORY_OVERFLOW(addr + offset, 1,
                                                       maddr);
                            CHECK_ATOMIC_MEMORY_ACCESS();
                            os_mutex_lock(&memory->mem_lock);
                            readv = (uint32)(*(uint8 *)maddr);
                            ...
                            CHECK_BULK_MEMORY_OVERFLOW(addr + offset, 2,
                                                       maddr);
                            CHECK_ATOMIC_MEMORY_ACCESS();
                            os_mutex_lock(&memory->mem_lock);
                            readv = (uint32)LOAD_U16(maddr);
                            ...
                            CHECK_BULK_MEMORY_OVERFLOW(addr + offset, 4,
                                                       maddr);
                            CHECK_ATOMIC_MEMORY_ACCESS();
                            os_mutex_lock(&memory->mem_lock);
                            readv = LOAD_I32(maddr);
                    ...
                    case WASM_OP_ATOMIC_I64_LOAD32_U:
                    {
                        ...
                        if (opcode == WASM_OP_ATOMIC_I64_LOAD8_U) {
                            ...
                            CHECK_BULK_MEMORY_OVERFLOW(addr + offset, 8,
                                                       maddr);
                            CHECK_ATOMIC_MEMORY_ACCESS();
                            os_mutex_lock(&memory->mem_lock);
                    ...
                    case WASM_OP_ATOMIC_RMW_I32_CMPXCHG16_U:
                    {
                        ...
                        if (opcode == WASM_OP_ATOMIC_RMW_I32_CMPXCHG8_U) {
                            ...
                            os_mutex_unlock(&memory->mem_lock);
                                 ...
                                 == WASM_OP_ATOMIC_RMW_I32_CMPXCHG16_U) {
                            CHECK_BULK_MEMORY_OVERFLOW(addr + offset, 2,
                                                       maddr);
                            CHECK_ATOMIC_MEMORY_ACCESS();
                    ...
                    case WASM_OP_ATOMIC_RMW_I64_CMPXCHG32_U:
                    {
                        ...
                        if (opcode == WASM_OP_ATOMIC_RMW_I64_CMPXCHG8_U) {
                            ...
                            os_mutex_unlock(&memory->mem_lock);
                                 ...
                                 == WASM_OP_ATOMIC_RMW_I64_CMPXCHG16_U) {
                            CHECK_BULK_MEMORY_OVERFLOW(addr + offset, 2,
                                                       maddr);
                            CHECK_ATOMIC_MEMORY_ACCESS();
                            ...
                            os_mutex_unlock(&memory->mem_lock);
                                 ...
                                 == WASM_OP_ATOMIC_RMW_I64_CMPXCHG32_U) {
                            CHECK_BULK_MEMORY_OVERFLOW(addr + offset, 4,
                                                       maddr);
                            CHECK_ATOMIC_MEMORY_ACCESS();
...
WASMModuleInstance *
wasm_interp_restore(uint32 **p_argv)
{
    WASMModuleInstance module_inst_dummy;
    WASMModule module_dummy;
    uint32 i;
    module_inst_dummy.default_memory = NULL;
    module_inst_dummy.module = &module_dummy;

    wasm_interp_call_func_bytecode(&module_inst_dummy, NULL, NULL, NULL, true);

    /* Output the return value to the caller */
    if (!wasm_get_exception(restore_module_inst)) {
        *p_argv = malloc(restore_function->ret_cell_num * sizeof(uint32));
        for (i = 0; i < restore_function->ret_cell_num; i++) {
            (*p_argv)[i] =
              *(restore_frame->sp + i - restore_function->ret_cell_num);
        }
    }
    else {
...
#endif
        LOG_DEBUG("meet an exception %s",
                  wasm_get_exception(restore_module_inst));
    }

    wasm_exec_env_set_cur_frame(restore_exec_env, restore_frame);
    FREE_FRAME(restore_exec_env, restore_frame);
    return restore_module_inst;
}
