         ...
         uint32 error_buf_size)
{
    ...
    if (!sign && maxbits == 32 && shift >= maxbits) {
        ...
        if (shift < maxbits) {
            /* Sign extend */
            result =
              (((int32)result) << (maxbits - shift)) >> (maxbits - shift);
        }
        else {
        ...
        if (shift < maxbits) {
            /* Sign extend */
            result =
              (((int64)result) << (maxbits - shift)) >> (maxbits - shift);
        }
        else {
            /* The top bits should be a sign-extension of the sign bit */
...
static char *
const_str_list_insert(const uint8 *str,
                      uint32 len,
                      WASMModule *module,
                      char *error_buf,
                      uint32 error_buf_size)
...
static bool
load_init_expr(const uint8 **p_buf,
               const uint8 *buf_end,
...
static bool
load_type_section(const uint8 *buf,
                  const uint8 *buf_end,
                  WASMModule *module,
                  char *error_buf,
                  uint32 error_buf_size)
                        ...
                        uint32 error_buf_size)
{
    ...
    if (export->index >= export_index_boundary) {
        LOG_DEBUG("%s in the module %s is out of index (%d >= %d )",
                  field_name, module_name, export->index,
...
static WASMTable *
wasm_loader_resolve_table(const char *module_name,
...
static WASMMemory *
wasm_loader_resolve_memory(const char *module_name,
...
reload_function_import(WASMModule *module)
{
    ...
    void *linked_func, *linked_attachment = NULL;
    ...
    for (i = 0; i < module->import_function_count; i++, import++) {
        linked_func = wasm_native_resolve_symbol(
          import->u.function.module_name, import->u.function.field_name,
...
static bool
load_function_import(const uint8 **p_buf,
                     const uint8 *buf_end,
...
static bool
check_table_max_size(uint32 init_size,
                     uint32 max_size,
                     char *error_buf,
                     uint32 error_buf_size)
{
...
static bool
load_table_import(const uint8 **p_buf,
                  const uint8 *buf_end,
...
static bool
check_memory_max_size(uint32 init_size,
                      uint32 max_size,
                      char *error_buf,
                      uint32 error_buf_size)
{
...
static bool
load_memory_import(const uint8 **p_buf,
                   const uint8 *buf_end,
                   WASMModule *parent_module,
                   const char *sub_module_name,
                   const char *memory_name,
...
static bool
load_global_import(const uint8 **p_buf,
                   const uint8 *buf_end,
                   const WASMModule *parent_module,
...
static bool
load_import_section(const uint8 *buf,
                    const uint8 *buf_end,
                    WASMModule *module,
                    char *error_buf,
                    uint32 error_buf_size)
{
    ...
    if (!const_str_list_insert((uint8 *)"env", 3, module, error_buf,
        ...
        || !const_str_list_insert((uint8 *)"wasi_unstable", 13, module,
                                  error_buf, error_buf_size)
        || !const_str_list_insert((uint8 *)"wasi_snapshot_preview1", 22,
...
static bool
load_function_section(const uint8 *buf,
                      const uint8 *buf_end,
                      const uint8 *buf_code,
                      const uint8 *buf_code_end,
                      WASMModule *module,
                      char *error_buf,
                      uint32 error_buf_size)
{
    ...
    if (func_count) {
        ...
        for (i = 0; i < func_count; i++) {
             ...
             */
            code_body_cp = loader_malloc(code_size, error_buf, error_buf_size);
            memcpy(code_body_cp, p_code, code_size);
            func->code = code_body_cp;
...
static bool
load_table_section(const uint8 *buf,
                   const uint8 *buf_end,
                   WASMModule *module,
                   char *error_buf,
                   uint32 error_buf_size)
...
static bool
load_memory_section(const uint8 *buf,
                    const uint8 *buf_end,
                    WASMModule *module,
                    char *error_buf,
                    uint32 error_buf_size)
...
static bool
load_global_section(const uint8 *buf,
                    const uint8 *buf_end,
                    WASMModule *module,
                    char *error_buf,
                    uint32 error_buf_size)
...
static bool
load_export_section(const uint8 *buf,
                    const uint8 *buf_end,
                    WASMModule *module,
                    char *error_buf,
                    uint32 error_buf_size)
...
static bool
check_table_index(const WASMModule *module,
                  uint32 table_index,
                  char *error_buf,
                  uint32 error_buf_size)
{
...
static bool
load_table_index(const uint8 **p_buf,
                 const uint8 *buf_end,
                 WASMModule *module,
                 uint32 *p_table_index,
                 char *error_buf,
                 uint32 error_buf_size)
{
    const uint8 *p = *p_buf, *p_end = buf_end;
...
#if WASM_ENABLE_REF_TYPES != 0
static bool
load_elem_type(const uint8 **p_buf,
               const uint8 *buf_end,
               uint32 *p_elem_type,
               bool elemkind_zero,
               char *error_buf,
...
static bool
load_func_index_vec(const uint8 **p_buf,
                    const uint8 *buf_end,
                    WASMModule *module,
...
static bool
load_table_segment_section(const uint8 *buf,
                           const uint8 *buf_end,
                           WASMModule *module,
                           char *error_buf,
                           uint32 error_buf_size)
...
static bool
load_data_segment_section(const uint8 *buf,
                          const uint8 *buf_end,
                          WASMModule *module,
                          char *error_buf,
                          uint32 error_buf_size)
...
#if WASM_ENABLE_BULK_MEMORY != 0
static bool
load_datacount_section(const uint8 *buf,
                       const uint8 *buf_end,
                       WASMModule *module,
                       char *error_buf,
                       uint32 error_buf_size)
...
static bool
load_code_section(const uint8 *buf,
                  const uint8 *buf_end,
...
static bool
load_start_section(const uint8 *buf,
                   const uint8 *buf_end,
                   WASMModule *module,
                   char *error_buf,
                   uint32 error_buf_size)
...
static bool
handle_name_section(const uint8 *buf,
                    const uint8 *buf_end,
                    WASMModule *module,
                    char *error_buf,
                    uint32 error_buf_size)
...
static bool
load_user_section(const uint8 *buf,
                  const uint8 *buf_end,
                  WASMModule *module,
                  char *error_buf,
                  uint32 error_buf_size)
...
static bool
load_from_sections(WASMModule *module,
    ...
    if (!module->possible_memory_grow) {
        ...
        if (aux_data_end_global && aux_heap_base_global
            ...
            if (module->memory_count) {
                ...
                init_memory_size =
                  (uint64)memory->num_bytes_per_page * memory->init_page_count;
                if (shrunk_memory_size <= init_memory_size) {
                    /* Reset memory info to decrease memory usage */
...
static uint8 section_ids[] = { SECTION_TYPE_USER,      SECTION_TYPE_TYPE,
...
#endif
                               SECTION_TYPE_CODE,      SECTION_TYPE_DATA };
...
static bool
create_sections(const uint8 *buf,
                uint32 size,
...
WASMModule *
wasm_loader_load(const uint8 *buf,
                 uint32 size,
                 char *error_buf,
                 uint32 error_buf_size)
{
...
#define MEM_REALLOC(mem, size_old, size_new)                                  \
    do {                                                                      \
        void *mem_new =                                                       \
          memory_realloc(mem, size_old, size_new, error_buf, error_buf_size); \
        if (!mem_new)                                                         \
            goto fail;                                                        \
        mem = mem_new;                                                        \
    } while (0)
...
static bool
check_stack_top_values(uint8 *frame_ref,
...
static bool
check_stack_pop(WASMLoaderContext *ctx,
                uint8 type,
                char *error_buf,
                uint32 error_buf_size)
{
...
static bool
wasm_loader_push_frame_ref(WASMLoaderContext *ctx,
                           uint8 type,
                           char *error_buf,
                           uint32 error_buf_size)
{
...
static bool
wasm_loader_pop_frame_ref(WASMLoaderContext *ctx,
                          uint8 type,
                          char *error_buf,
                          uint32 error_buf_size)
{
...
static bool
wasm_loader_push_frame_csp(WASMLoaderContext *ctx,
                           uint8 label_type,
                           BlockType block_type,
                           uint8 *start_addr,
                           char *error_buf,
...
static bool
preserve_referenced_local(WASMLoaderContext *loader_ctx,
                          uint8 opcode,
                          uint32 local_index,
                          uint32 local_type,
                          bool *preserved,
...
static bool
wasm_loader_emit_br_info(WASMLoaderContext *ctx,
                         BranchBlock *frame_csp,
                         char *error_buf,
                         uint32 error_buf_size)
{
...
static bool
wasm_loader_push_frame_offset(WASMLoaderContext *ctx,
                              uint8 type,
                              bool disable_emit,
...
static bool
wasm_loader_pop_frame_offset(WASMLoaderContext *ctx,
                             uint8 type,
                             char *error_buf,
                             uint32 error_buf_size)
{
...
static bool
wasm_loader_push_pop_frame_offset(WASMLoaderContext *ctx,
                                  uint8 pop_cnt,
                                  uint8 type_push,
                                  uint8 type_pop,
                                  bool disable_emit,
...
static bool
wasm_loader_push_frame_ref_offset(WASMLoaderContext *ctx,
                                  uint8 type,
                                  bool disable_emit,
                                  int16 operand_offset,
                                  char *error_buf,
                                  uint32 error_buf_size)
{
    if (!(wasm_loader_push_frame_offset(
          ctx, type, disable_emit, operand_offset, error_buf, error_buf_size)))
        return false;
...
static bool
wasm_loader_pop_frame_ref_offset(WASMLoaderContext *ctx,
                                 uint8 type,
                                 char *error_buf,
                                 uint32 error_buf_size)
{
...
static bool
wasm_loader_push_pop_frame_ref_offset(WASMLoaderContext *ctx,
                                      uint8 pop_cnt,
                                      uint8 type_push,
                                      uint8 type_pop,
                                      bool disable_emit,
...
static bool
wasm_loader_get_const_offset(WASMLoaderContext *ctx,
                             uint8 type,
                             void *value,
                             int16 *offset,
                             char *error_buf,
...
#define POP_CSP()                                                             \
    do {                                                                      \
        if (!wasm_loader_pop_frame_csp(loader_ctx, error_buf,                 \
                                       error_buf_size))                       \
            goto fail;                                                        \
    } while (0)
...
static bool
check_memory_access_align(uint8 opcode,
                          uint32 align,
                          char *error_buf,
                          uint32 error_buf_size)
{
...
#if (WASM_ENABLE_WAMR_COMPILER != 0) || (WASM_ENABLE_JIT != 0)
static bool
check_simd_memory_access_align(uint8 opcode,
                               uint32 align,
                               char *error_buf,
                               uint32 error_buf_size)
{
...
static bool
check_simd_access_lane(uint8 opcode,
                       uint8 lane,
                       char *error_buf,
                       uint32 error_buf_size)
{
...
#if WASM_ENABLE_SHARED_MEMORY != 0
static bool
check_memory_align_equal(uint8 opcode,
                         uint32 align,
                         char *error_buf,
                         uint32 error_buf_size)
{
...
static bool
check_block_stack(WASMLoaderContext *loader_ctx,
                  BranchBlock *block,
                  char *error_buf,
                  uint32 error_buf_size)
{
 ...
 */
static bool
copy_params_to_dynamic_space(WASMLoaderContext *loader_ctx,
                             bool is_if_block,
                             char *error_buf,
                             uint32 error_buf_size)
{
    ...
    uint64 size =
      (uint64)param_count * (sizeof(*cells) + sizeof(*src_offsets));
                    ...
                    uint32 error_buf_size)
{
    ...
    if (p_elem_type) {
        if (table_idx < module->import_table_count)
            ...
            *p_elem_type =
              module->tables[module->import_table_count + table_idx].elem_type;
    }
    return true;
}
  ...
  && WASM_ENABLE_JIT == 0
...
#endif
            {
                ...
                if (func_idx < module->import_function_count)
                    ...
                    func_type =
                      module
                        ->functions[func_idx - module->import_function_count]
                        ->func_type;
